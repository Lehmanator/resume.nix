{lib, ...}: rec {
  ...
  date.iso8601 = lib.types.strMatching "^([1-2][0-9]{3}-[0-1][0-9]-[0-3][0-9]|[1-2][0-9]{3}-[0-1][0-9]|[1-2][0-9]{3})$";
  ...
  uri = url;
  urlSlug = lib.types.strMatching "^[a-z0-9-]+$";
  ...
  httpURL = lib.types.strMatching "^https?:\/\/[\w\.\/\-?=&%,]+$";
  ...
  # TODO: Phone number
  phoneNumber = lib.types.strMatching "^(\+[0-9]+)$";

  username = lib.types.strMatching "^[a-zA-Z0-9_-]{3,16}$";
  ...
  postalCodeBrazil = lib.types.strMatching "^[0-9]{5}-[0-9]{3}$";

  # --- Versioning ---------------------
  semVer = lib.types.strMatching "^v?((0|[1-9]+[0-9]*)\.){3}$";
}
