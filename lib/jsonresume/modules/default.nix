      ...
      options = {
        ...
        result = mkOption {
          ...
          type = let
            ...
            data = rec {
              jsonresume = builtins.removoeAttrs (documentArgs.config ["type" "result" "settings" "themeSettings"]);
          ...
          in
            types.submodule {
              options = let
                ...
                builders = {
                  # Builds a JSON Resume using `pkgs.resumed`
                  html.resumed = theme:
                    pkgs.callPackage ../../../packages/jsonresume/scripts/builder-html-resumed.nix {
                      inherit theme;
                      src = documentArgs.config.assetsDirectory;
                      version = data.jsonresume.meta.version or "v0.0.1";
                      json = documentArgs.config.result.jsonresume.json.outPath;
                      extraThemes = [];
                    };
                  html.goresume = theme:
                    pkgs.callPackage ../../../packages/jsonresume/scripts/builder-html-goresume.nix {
                      inherit theme;
                      extraThemes = [];
                      src = documentArgs.config.assetsDirectory;
                      version = data.jsonresume.meta.version or "v0.0.1";
                  ...
                  pdf.goresume = theme:
                    pkgs.callPackage ../../../packages/jsonresume/scripts/builder-pdf-goresume.nix {
                      inherit theme;
                      extraThemes = [];
                      src = documentArgs.config.assetsDirectory;
                      version = data.jsonresume.meta.version or "v0.0.1";
                ...
                themes = {
                  jsonresume = builtins.map (theme: pkgs) ["stackoverflow"];
                  goresume = ["actual" "simple" "simple-compact"];
                  hackmyresume = [""];
                  rendercv = [""];
                };
              in {
                ...
                html = mkOption {
                  ...
                  default = builtins.mapAttrs (theme: v:
                    pkgs.callPackage ../../../packages/jsonresume/themes/${theme}.nix {
                      inherit theme;
                      data = data.jsonresume;
                    });
                };
                docx = mkOption {type = types.nullOr (types.attrsOf types.package);};
                latex = mkOption {type = types.nullOr (types.attrsOf types.package);};
                pdf = mkOption {type = types.nullOr (types.attrsOf types.package);};
                ...
                hackmyresume = {
                  nix = mkOption {
                    ...
                    default = pkgs.writeText "${filePrefix}.jsonresume.nix" data.hackmyresume;
                    type = types.package;
                  };
                  ini = mkOption {
                ...
                jsonresume = {
                  nix = mkOption {
                    description = "JSON Resume in Nix format";
                    defaultText = "Final JSON Resume input data in format: Nix";
                    default = pkgs.writeText "${filePrefix}.jsonresume.nix" data.jsonresume;
                    type = types.package;
                  };
                  ini = mkOption {
                    description = "JSON Resume in INI format";
                    defaultText = "Final JSON Resume input data in format: INI";
                    default = (pkgs.formats.ini {}).generate "${filePrefix}.jsonresume.ini" data.jsonresume;
                    type = types.package;
                  };
                  json = mkOption {
                    description = "JSON Resume in JSON format";
